...
class PinVerificationPage extends StatefulWidget {
  ...
  const PinVerificationPage({super.key, this.isSetup = false});
    ...
    if (widget.isSetup) {
      if (_pinController.text.length < 4) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Por seguridad, el PIN debe tener al menos 4 dígitos'),
      ...
      if (_pinController.text != _confirmPinController.text) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Los PINs ingresados no coinciden, verifica e intenta nuevamente'),
            backgroundColor: Colors.orange,
          ),
        );
        return;
      ...
      if (_pinController.text.isEmpty) {
        setState(() {
          _errorText = 'Por favor, ingresa tu PIN';
        });
        return;
      }
      ...
      body: BlocConsumer<AuthBloc, AuthState>(
        listener: (context, state) {
          ...
          if (state is AuthenticatedState || state is PinSavedState) {
            ...
            // Mostrar mensaje con feedback visual
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('El PIN ingresado no es correcto'),
